...
from dymos.examples.racecar.tracks import Barcelona  # track curvature imports
...
phase = dm.Phase(ode_class=CombinedODE,
                 transcription=dm.GaussLobatto(num_segments=150, order=3, compressed=True))
...
#phase.add_path_constraint('V', upper=50, ref=50)  # Top limit
...
phase.add_parameter('M', val=1900.0, units='kg', opt=False,
...
phase.add_parameter('chi', val=0.33, units=None, opt=False, targets=['normal.chi'],
                    static_target=True)  # roll stiffness
phase.add_parameter('ClA', val=1.0, units='m**2', opt=False, targets=['normal.ClA'],
                    static_target=True)  # downforce coefficient*area
phase.add_parameter('CdA', val=1.0, units='m**2', opt=False, targets=['car.CdA'],
...
p.set_val('traj.phase0.states:E', phase.interp('E', [0.0, 0.0]), units='J')
...
# delta vs s
axes[3].plot(s,
             p.get_val('traj.phase0.timeseries.states:E', units=None),
             label='solution')
...
# Plot the main vehicle telemetry vs time
fig, axes = plt.subplots(nrows=4, ncols=1, figsize=(15, 8))
...
axes[1].set_ylabel('n (m)')
axes[1].grid()
axes[1].set_xlim(0, max(time))

# throttle vs s
axes[2].plot(time, thrust)

axes[2].set_xlabel('time (s)')
axes[2].set_ylabel('thrust')
axes[2].grid()
axes[2].set_xlim(0, max(time))
